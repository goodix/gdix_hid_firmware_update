...
#define GTPUPDATE_GETOPTS	"hfd:pvt:s:imca:"
...
extern int gdix_do_fw_update(const char *devname, const char *filename, uint8_t i2c_addr);
...
static void printHelp(const char *prog_name)
{
	...
	fprintf(stdout, "\t-a, --i2c-addr\t if this option is set, will be upgraded by i2c.(only support berlinB)\n");
}
...
int main(int argc, char **argv)
{
	...
	if(pid != NULL) {
		...
		else if (REG_NOERROR == regexec(&reg_x9xx, productionTypeName, 1, pamtch, 0))
