...
GTx8Update::GTx8Update() { is_cfg_flashed_with_isp = false; }
...
int GTx8Update::DisableReport()
{
	unsigned char cmdDisable[] = {0x33, 0x00, 0xCD};
	unsigned char cmdConfirm[] = {0x35, 0x00, 0xCB};
	unsigned char buf[3] = {0};
	int ret;

	for (int i = 0; i < 3; i++) {
		ret = dev->Write(CMD_ADDR, cmdDisable, sizeof(cmdDisable));
		if (ret < 0) {
			gdix_err("send close report cmd failed\n");
			return ret;
		}
		usleep(10000);
	}

	ret = dev->Write(CMD_ADDR, cmdConfirm, sizeof(cmdConfirm));
	if (ret < 0) {
		gdix_err("send confirm cmd failed\n");
		return ret;
	}
	usleep(30000);
	ret = dev->Read(CMD_ADDR, buf, sizeof(buf));
	if (ret < 0) {
		gdix_err("read confirm flag failed\n");
		return ret;
	}

	if (buf[1] != 1) {
		gdix_err("close report failed, flag[0x%02X]\n", buf[1]);
		return -EINVAL;
	}

	gdix_info("close report success\n");
	return 0;
}
...
int GTx8Update::fw_update(unsigned int firmware_flag)
{
	...
	if (dev->WriteSpeCmd(buf_switch_ptp_mode, sizeof(buf_switch_ptp_mode)) <
		0) {
