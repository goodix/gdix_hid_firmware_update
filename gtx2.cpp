...
int GTx2Device::GetFirmwareProps(const char *deviceName, char *props_buf, int len)
{
	int ret;
...
int GTx2Device::Open(const char *filename)
{
	int rc;
...
int GTx2Device::GetReport(unsigned char reportId, unsigned char *buf)
{
	int ret;
...
int GTx2Device::Read(unsigned short addr, unsigned char *buf, unsigned int len)
{
	int ret;
	int retry = 0;
...
int GTx2Device::Write(unsigned short addr, const unsigned char *buf,
				 unsigned int len)
{
 ...
 * return < 0 failed
*/
int GTx2Device::Write(const unsigned char *buf,
				 unsigned int len)
{
...
int GTx2Device::SetBasicProperties()
{
	...
	do {
		ret = Read(GTX2_VERSION_ADDR, fw_info, sizeof(fw_info));
		if (ret < 0)
