...
public:
	...
	virtual int Read(unsigned int addr, unsigned char *buf, unsigned int len)
	{
		return 0;
	}
	virtual int GetReport(unsigned char reportId, unsigned char *buf)
	{
		return 0;
	}

	virtual int Write(unsigned int addr, const unsigned char *buf,
					  unsigned int len)
	{
		return 0;
	}
	virtual int Write(const unsigned char *buf, unsigned int len) { return 0; }
	virtual int WriteSpeCmd(const unsigned char *buf, unsigned int len)
	{
		return 0;
	}
	virtual int SendCmd(unsigned char cmd, unsigned char *data, int dataLen)
	{
		return 0;
	}
	virtual int SendConfig(unsigned char *config, int len) { return 0; }

	virtual int GetFirmwareProps(const char *deviceName, char *props_buf,
								 int len)
	{
		return 0;
	}
