...
bool GTx9Device::IsOpened()
{
    return m_deviceOpen;
}

int GTx9Device::GetFd()
{
    return m_fd;
}
...
int GTx9Device::Write(unsigned int addr, const unsigned char *buf, unsigned int len)
{
    uint8_t HidBuf[PACKAGE_LEN] = {0};
...
int GTx9Device::GetFirmwareProps(const char *deviceName, char *props_buf, int len)
{
    int ret;
    ...
    snprintf(props_buf, len, "%d.%d", GetFirmwareVersionMajor(), GetFirmwareVersionMinor());
    return 0;
}
...
int GTx9Device::SetBasicProperties()
{
    ...
    gdix_info("PID:%s\n", m_pid);
    gdix_info("VID:%02x %02x %02x %02x\n", m_vid[0], m_vid[1], m_vid[2], m_vid[3]);
